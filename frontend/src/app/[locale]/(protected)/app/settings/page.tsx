...
/**
 * Settings hub: identity (read-only), authenticated password change, language, theme,
 * study preferences, and quick links to data tools (export/import, stats, study health).
 *
 * Password change calls `POST /api/user/change-password`; on success the API returns a new access
 * token + user payload — we call `setAuthSuccess` so the client session updates without a reload.
 * Failed attempts surface API/validation messages via `getApiErrorMessage`.
 *
 * URL note: the legacy `/app/account` route now redirects here. Keep this single hub so users
 * have one obvious place for everything settings-shaped (sidebar entry was removed in 1.5.0;
 * the header user dropdown is the canonical entry point).
 */
  ...
  return (
    <div className="mc-study-page mx-auto max-w-2xl space-y-8">
      <div>
        ...
        <h2 className="mt-2 text-xl font-semibold text-(--mc-text-primary)">{tc('settings')}</h2>
      ...
      <section className="rounded-xl border border-(--mc-border-subtle) bg-(--mc-bg-card) p-6 shadow-sm">
        ...
        <form onSubmit={handleChangePassword} className="mt-4 max-w-md space-y-3" autoComplete="on" method="post" action="#">
          ...
          <div>
            ...
            <PasswordStrengthMeter password={newPassword} minLength={PASSWORD_MIN_LENGTH} t={tc} />
          </div>
          <div>
          ...
          <Button type="submit" variant="primary" disabled={pwdSaving}>
            {pwdSaving && <Spinner size="xs" className="mr-1.5" />}
      ...
      <section className="rounded-xl border border-(--mc-border-subtle) bg-(--mc-bg-card) p-6 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="mt-4">
          ...
          <Button type="submit" variant="primary" className="mt-4" disabled={settingsLoading || saving}>
            {saving && <Spinner size="xs" className="mr-1.5" />}
